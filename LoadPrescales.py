...
class LoadPrescales:
    def __init__(self):
        ...
        #        self.configname = "/users/jjhollar/July73E33MenuValidation/CMSSW_4_2_0_HLT20_Master_V752_Presc
alesV1/V1"
        #        self.prescalefile = "hltprescales_3E33_July7.txt"
        self.configname = "/users/jjhollar/July73E33MenuValidation/CMSSW_4_2_0_HLT20_Master_V752_PrescalesV1/V3
"
    ...
    def BeginJob(self):
        ...
        # Get the menu
        self.dbcursor.execute("SELECT Configurations.configId FROM Configurations WHERE (configDescriptor = '" 
+ self.configname + "')")
        tmprelid = (self.dbcursor.fetchone())[0]

        # Get a list of all HLT paths in the menu
        self.dbcursor.execute("SELECT Paths.name FROM Paths JOIN ConfigurationPathAssoc ON Paths.pathId = Confi
gurationPathAssoc.pathId WHERE ConfigurationPathAssoc.configId = " + str(tmprelid))
        ...
        self.dbcursor.execute("SELECT Services.superId FROM Services JOIN ServiceTemplates ON Services.template
Id = ServiceTemplates.superId JOIN ConfigurationServiceAssoc ON ConfigurationServiceAssoc.serviceId = Services.
superId WHERE ServiceTemplates.name = 'PrescaleService' AND ConfigurationServiceAssoc.configId = " + str(tmprel
id))
        presid = (self.dbcursor.fetchone())[0]

        # Get the Prescale Table VPSet
        self.dbcursor.execute("SELECT VecParameterSets.superId FROM VecParameterSets JOIN SuperIdVecParamSetAss
oc ON VecParameterSets.superId = SuperIdVecParamSetAssoc.vpsetId WHERE VecParameterSets.name = 'prescaleTable' 
AND SuperIdVecParamSetAssoc.superId = " + str(presid))
        pretableid = (self.dbcursor.fetchone())[0]

        # Get the PSet for each trigger 
        self.dbcursor.execute("SELECT ParameterSets.superId FROM ParameterSets JOIN SuperIdParamSetAssoc ON Sup
erIdParamSetAssoc.psetId = ParameterSets.superId JOIN VecParameterSets ON VecParameterSets.superId = SuperIdPar
amSetAssoc.superId WHERE VecParameterSets.superId = " + str(pretableid))
        pretablepsetids = (self.dbcursor.fetchall())

        # Get the trigger name
        self.dbcursor.execute("SELECT StringParamValues.value FROM StringParamValues JOIN SuperIdParameterAssoc
 ON SuperIdParameterAssoc.paramId = StringParamValues.paramId WHERE SuperIdParameterAssoc.superId = " + str(pre
sid))
        defaultlabel = (self.dbcursor.fetchone())[0]

        # Get the Prescale set 
        self.dbcursor.execute("SELECT VStringParamValues.value FROM VStringParamValues JOIN SuperIdParameterAss
oc ON SuperIdParameterAssoc.paramId = VStringParamValues.paramId WHERE SuperIdParameterAssoc.superId = " + str(
presid))
        lvl1labels = (self.dbcursor.fetchall())

        # Indices of PrescaleService entries, for ConfDB Sequence number bookkeeping
        self.dbcursor.execute("SELECT SuperIdParamSetAssoc.sequenceNb FROM SuperIdParamSetAssoc JOIN VecParamet
erSets ON VecParameterSets.superId = SuperIdParamSetAssoc.superId WHERE VecParameterSets.superId = " + str(pret
ableid))
        ...
        for hltpath in self.hltpathsinmenu:
            ...
            for pretablepsetidpair in pretablepsetids:
                ...
                self.dbcursor.execute("SELECT StringParamValues.value FROM StringParamValues JOIN SuperIdParame
terAssoc ON SuperIdParameterAssoc.paramId = StringParamValues.paramId WHERE SuperIdParameterAssoc.superId = " +
 str(pretablepsetid))
                ...
                self.dbcursor.execute("SELECT VUInt32ParamValues.paramId FROM VUInt32ParamValues JOIN SuperIdPa
rameterAssoc ON SuperIdParameterAssoc.paramId = VUInt32ParamValues.paramId WHERE SuperIdParameterAssoc.superId 
= " + str(pretablepsetid))
                vectorparamid = (self.dbcursor.fetchone())[0]
            
                self.dbcursor.execute("SELECT VUInt32ParamValues.value FROM VUInt32ParamValues JOIN SuperIdPara
meterAssoc ON SuperIdParameterAssoc.paramId = VUInt32ParamValues.paramId WHERE SuperIdParameterAssoc.superId = 
" + str(pretablepsetid))
                ...
                for prescale in prescales: 
                    if((lvl1labels[i][0]).find("Cosmics") == -1):
                        newprescale = newprescales[i]
                        print "\tChange " + str(lvl1labels[i][0]) + " prescale of " + str(trigname) + " from " 
+ str(prescale[0]) + " to " + str(newprescale)
                        if(self.cliupdate == 1):
                            self.dbcursor.execute("UPDATE VUInt32ParamValues SET value='" + str(newprescale) + 
"' WHERE VUInt32ParamValues.sequenceNb = " + str(i) + " AND VUInt32ParamValues.paramId = " + str(vectorparamid)
) 
            ...
            if((foundtrigger == False) and (int(maxprescale) > 1)):
                print "Prescaled trigger " + str(hltpath) + " does not have a non-trivial prescale set - new en
try needed!"
                ...
                self.dbcursor.execute("INSERT INTO ParameterSets (superId, name, tracked) VALUES (" + str(newpa
ramsetid) + ", '', " + str(1) + ")")
                self.dbcursor.execute("INSERT INTO SuperIdParamSetAssoc (superId, psetId, sequenceNb) VALUES ("
 + str(pretableid) + ", " + str(newparamsetid) + ", " + str(j+k) + ")")

                # Add String - be sure to use the correctly versioned name taken from the DB
                self.dbcursor.execute("INSERT INTO Parameters (paramTypeId, name, tracked) VALUES (" + str(8) +
 ", 'pathName', " + str(1) + ")")
                self.dbcursor.execute("SELECT ParamId_Sequence.currval from dual")
                newparamid = self.dbcursor.fetchone()[0]
                self.dbcursor.execute("INSERT INTO StringParamValues (paramId, value) VALUES (" + str(newparami
d) + ", '" + str(hltpath) + "')")
                self.dbcursor.execute("INSERT INTO SuperIdParameterAssoc (superId, paramId, sequenceNb) VALUES 
(" + str(newparamsetid) + ", " + str(newparamid) + ", " + str(0) + ")")

                # Add Prescale set
                self.dbcursor.execute("INSERT INTO Parameters (paramTypeId, name, tracked) VALUES (" + str(5) +
 ", 'prescales', " + str(1) + ")")
                self.dbcursor.execute("SELECT ParamId_Sequence.currval from dual")
                newparamid = self.dbcursor.fetchone()[0]
                self.dbcursor.execute("INSERT INTO SuperIdParameterAssoc (superId, paramId, sequenceNb) VALUES 
(" + str(newparamsetid) + ", " + str(newparamid) + ", " + str(1) + ")")
                ...
                m = 0
                for prescale in prescales:
                    if((lvl1labels[m][0]).find("Cosmics") == -1):
                        newprescale = newprescales[m]
                        print "\tSet " + str(lvl1labels[m][0]) + " prescale of " + str(hltpath) + " to " + str(
newprescale)
                        if(self.cliupdate == 1):
-bash-4.1$ clear
